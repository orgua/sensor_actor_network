...
private:
    ...
    static constexpr uint16_t CRCINIT       = (0xffff);
...
public:
    ...
    void write_tailer(stack_message& msg)
    {
        //msg.position_end += 2;
        tailer crc;
        crc.value = crcCalculation(msg, header_position, msg.position);
        msg.add_payload(crc.form.checksumH);
        msg.add_payload(crc.form.checksumL);
    };
    ...
    {
        return ((crc >> 8) ^ FCSTAB[(crc ^ _b) & 0xff]);
    };

    inline uint16_t crcCalculation(const stack_message& msg, const uint8_t startPosition, const uint8_t endPosition)
    {
        uint16_t crc = CRCINIT;
        for (uint8_t loopvar = startPosition; loopvar < endPosition; ++loopvar)
        {
            crc = crcIteration(crc,msg.payload[loopvar]);
        }
        return crc;
    }
